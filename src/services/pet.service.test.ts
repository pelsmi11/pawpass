...
import { createPetWithValidation } from "./pet.service";

describe("pet.service - createPetWithValidation", () => {
  ...
  it("returns fieldErrors when petTypeId does not exist", async () => {
    ...
    if (!result.success) {
      expect(result.fieldErrors.petTypeId).toBeDefined();
    }
  ...
  it("rejects sessionId in body", async () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.fieldErrors.sessionId).toBe("No permitido.");
    }
  });
