...
import { validatePetInput } from "./validation";
...
describe("validatePetInput", () => {
  ...
  it("rejects ownerName with only spaces", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) expect(result.fieldErrors.ownerName).toBeDefined();
  });
  ...
  it("rejects age -1", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) expect(result.fieldErrors.age).toBeDefined();
  });
  ...
  it("rejects petTypeId non-uuid", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) expect(result.fieldErrors.petTypeId).toBeDefined();
  });
  ...
  it("rejects strict sessionId in body", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) expect(result.fieldErrors.sessionId).toBeDefined();
  });
